...
class RoomController {
  async createSingleChat(req: Express.Request, res: Express.Response) {
    try {
      ...
      if (
        user_id === partner_id ||
        req.user?._id.toString() === partner_id.toString()
      )
        return res.json(
          new ResponseError(
            "Can not create room failed because missing partner"
          )
        );
      ...
      return res.json(
        new ResponseData(roomRes, "create room chat successfully")
      );
    } catch (error) {
      log.error(error);
  ...
  async createGroupChat(req: Express.Request, res: Express.Response) {
    try {
      ...
      if (await RoomService.getRoomByDependId(params.depend_id)) {
        return res.json(
          new ResponseError("This depend id already contain room chat")
        );
      }

      if (memberIds?.length < 2) {
        return res.json(
          new ResponseError("Group chat must has atleast two members")
        );
      }
      ...
      return res.json(
        new ResponseData(roomRes, "Create group chat successfully")
      );
    } catch (error) {
      log.error(error);
      return res.status(400).send(error);
  ...
  async getRoomDetail(req: Express.Request, res: Express.Response) {
    try {
      ...
      const room = await RoomService.getRoomDetail({
      ...
      if (!room)
        return res.json(
          new ResponseError(
            "Room not found, the room has been deleted or changed"
          )
        );

      if (
        !room.members?.data?.some(
          (item) => item.user_id.toString() === req.user._id.toString()
        )
      )
        return res.json(
          new ResponseError(
            "You are not in this room, so you can not get detail"
          )
        );
  ...
  async softDeleteRoom(req: Express.Request, res: Express.Response) {
    try {
      if (req.user?.role === "customer" && req.room.room_type === "single") {
        ...
        if (!room)
          return res.json(new ResponseError("Failed to soft delete room"));

        setTimeout(async () => {
          const users = await RoomService.getSocketIdsFromRoom(
            room._id.toString()
          );
          users.forEach((item) => {
            if (
              item?.socket_id &&
              item.user_id.toString() !== req.user._id.toString()
            ) {
        ...
        return res.json(
          new ResponseData({ room_id: req.params.room_id }, "Soft deleted room")
        );
      }
    } catch (error) {
      log.error(error);
  ...
  async softDeleteRoomsByDependId(req: Express.Request, res: Express.Response) {
    try {
      ...
      if (socketIds === undefined)
        return res.json(
          new ResponseError("This depend id does not contain any room chat")
        );
  ...
  async restoreSoftDeleteRoomByDependId(
    req: Express.Request,
  ...
  async restoreSoftDeleteRoom(req: Express.Request, res: Express.Response) {
    try {
      ...
      if (!status)
        return res.json(new ResponseError("Failed to restore this room"));
  ...
  async addMessageUnReadToRoom(req: Express.Request, res: Express.Response) {
    try {
      ...
      if (!room)
        return res.json(
          new ResponseError("Failed to add message unread to room")
        );
  ...
  async clearMessageUnreadFromRoom(
    req: Express.Request,
    res: Express.Response
  ) {
    try {
      ...
      if (!data)
        return res.json(
          new ResponseError("Failed to clear message unread from room")
        );

      return res.json(
        new ResponseData(
          { message_unread_count: 0 },
          "Cleared message unread from room"
        )
      );
    } catch (error) {
      log.error(error);
  ...
  async leaveRoom(req: Express.Request, res: Express.Response) {
    try {
      ...
      if (room.room_type === "single")
        return res.json(
          new ResponseError("This is single chat room, so you can not leave")
        );

      if (
        !room.member_ids?.some(
  ...
  async joinRoom(req: Express.Request, res: Express.Response) {
    try {
      ...
      if (room.room_type === "single")
        return res.json(
          new ResponseError("This is single room chat, so you can not join in")
        );
      ...
      if (socketIds === undefined)
        return res.json(new ResponseError("Failed to join room"));
  ...
  async updateRoomInfo(req: Express.Request, res: Express.Response) {
    try {
      ...
      if (!room)
        return res.json(new ResponseError("Failed to update room info"));
  ...
  async pinMessageToRoom(req: Express.Request, res: Express.Response) {
    try {
      ...
      if (!room)
        return res.json(new ResponseError("Failed to pin message to room"));
  ...
  async deleteMessagePinnedFromRoom(
    req: Express.Request,
    res: Express.Response
  ) {
    try {
